	...
	_, err = data.Query("CREATE TABLE IF NOT EXISTS yes (id INTEGER PRIMARY KEY AUTOINCREMENT,  age int NULL  , email text NULL  , name text NULL )")
	if err != nil {
		fmt.Println(err)
	}
	
	//get from database
	rows, err := data.Query("select * from yes")
	if err != nil {
		fmt.Println(err)
	}
